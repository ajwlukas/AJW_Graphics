...
bool FBXLibraryInternal::Load(std::string filename)
{
...
void FBXLibraryInternal::LoadMaterial(TL_FBXLibrary::MeshInfo& meshInfo, FbxSurfaceMaterial* fbxMaterialData, std::vector<TL_FBXLibrary::FBXMaterialInfo>& matInfo)
{
	...
	if (fbxMaterialData->GetClassId().Is(FbxSurfacePhong::ClassId))
	{
		...
		_materialInfo.roughness = 1.f - (sqrt(max(_materialInfo.shininess, 0.f)) / 10.f);
...
bool FBXLibraryInternal::TestCode()
{
	Load(TEXT("_DevelopmentAssets/Model/Joyful_Jump.fbx"));
	AllConvertOptimize();

	BinaryLoad(TEXT("../../../../../4_Resources/_DevelopmentAssets/Model/Joyful_Jump.TL"));

	bool _result = true;

	if (_result)
	{
		for (int i = 0; i < m_pFBXPrefab->m_MaterialList.size(); ++i)
		{
			const auto _fbxMat = m_pFBXPrefab->m_MaterialList[i];
			const auto _biMat = m_bineryFrefab->m_MaterialList[i];

			_fbxMat.materialIndex == _biMat.materialIndex ? _result = true : _result = false;
			_fbxMat.materialName == _biMat.materialName ? _result = true : _result = false;

			for (int i = 0; i < 4; ++i)
			{
				_fbxMat.ambient[i] == _fbxMat.ambient[i] ? _result = true : _result = false;
				_fbxMat.diffuse[i] == _fbxMat.diffuse[i] ? _result = true : _result = false;
				_fbxMat.specular[i] == _fbxMat.specular[i] ? _result = true : _result = false;
				_fbxMat.emissive[i] == _fbxMat.emissive[i] ? _result = true : _result = false;
			}

			_fbxMat.roughness == _fbxMat.roughness ? _result = true : _result = false;
			_fbxMat.shininess == _fbxMat.shininess ? _result = true : _result = false;
			_fbxMat.transparency == _fbxMat.transparency ? _result = true : _result = false;
			_fbxMat.reflectivity == _fbxMat.reflectivity ? _result = true : _result = false;

			_fbxMat.baseColorFile == _fbxMat.baseColorFile ? _result = true : _result = false;
			_fbxMat.normalMapFile == _fbxMat.normalMapFile ? _result = true : _result = false;
			_fbxMat.roughnessMapFile == _fbxMat.roughnessMapFile ? _result = true : _result = false;
			_fbxMat.emissiveFile == _fbxMat.emissiveFile ? _result = true : _result = false;
			_fbxMat.ambientFile == _fbxMat.ambientFile ? _result = true : _result = false;
			_fbxMat.specularFile == _fbxMat.specularFile ? _result = true : _result = false;
		}

		for (int i = 0; i < m_pFBXPrefab->m_MeshList.size(); ++i)
		{
			const auto _fbxMesh = m_pFBXPrefab->m_MeshList[i];
			const auto _biMesh = m_bineryFrefab->m_MeshList[i];

			_fbxMesh.meshNodeName == _biMesh.meshNodeName ? _result = true : _result = false;
			_fbxMesh.isMaterial == _fbxMesh.isMaterial ? _result = true : _result = false;
			_fbxMesh.isStatic == _fbxMesh.isStatic ? _result = true : _result = false;
			_fbxMesh.isAnimation == _fbxMesh.isAnimation ? _result = true : _result = false;

			_fbxMesh.materialIndexs == _fbxMesh.materialIndexs ? _result = true : _result = false;

			if (_fbxMesh.StaticVertex.size() != 0)
			{
				for (int i = 0; i < _fbxMesh.StaticVertex.size(); ++i)
				{
					_fbxMesh.StaticVertex[i].pos == _biMesh.StaticVertex[i].pos ? _result = true : _result = false;
					_fbxMesh.StaticVertex[i].uv == _biMesh.StaticVertex[i].uv ? _result = true : _result = false;
					_fbxMesh.StaticVertex[i].normal == _biMesh.StaticVertex[i].normal ? _result = true : _result = false;
					_fbxMesh.StaticVertex[i].tangent == _biMesh.StaticVertex[i].tangent ? _result = true : _result = false;
					_fbxMesh.StaticVertex[i].bitangent == _biMesh.StaticVertex[i].bitangent ? _result = true : _result = false;
				}
			}
			else
			{
				for (int i = 0; i < _fbxMesh.SkeletalVertex.size(); ++i)
				{
					_fbxMesh.SkeletalVertex[i].pos == _biMesh.SkeletalVertex[i].pos ? _result = true : _result = false;
					_fbxMesh.SkeletalVertex[i].uv == _biMesh.SkeletalVertex[i].uv ? _result = true : _result = false;
					_fbxMesh.SkeletalVertex[i].normal == _biMesh.SkeletalVertex[i].normal ? _result = true : _result = false;
					_fbxMesh.SkeletalVertex[i].tangent == _biMesh.SkeletalVertex[i].tangent ? _result = true : _result = false;
					_fbxMesh.SkeletalVertex[i].bitangent == _biMesh.SkeletalVertex[i].bitangent ? _result = true : _result = false;

					_fbxMesh.SkeletalVertex[i].index == _biMesh.SkeletalVertex[i].index ? _result = true : _result = false;
					_fbxMesh.SkeletalVertex[i].weight1 == _biMesh.SkeletalVertex[i].weight1 ? _result = true : _result = false;
				}
			}

			for (int i = 0; i < _fbxMesh.indexBuffer.size(); ++i)
			{
				for (int j = 0; j < _fbxMesh.indexBuffer[i].first.size(); ++j)
				{
					_fbxMesh.indexBuffer[i].first[j].index == _biMesh.indexBuffer[i].first[j].index ? _result = true : _result = false;
				}
			}
		}
		...
		for (int i = 0; i < m_pFBXPrefab->m_ObjNodeList.size(); ++i)
		{
			const auto _fbxNode = m_pFBXPrefab->m_ObjNodeList;
			const auto _biNode = m_bineryFrefab->m_ObjNodeList;

			_fbxNode[i].nodeName == _biNode[i].nodeName ? _result = true : _result = false;
			_fbxNode[i].parentNodeName == _biNode[i].parentNodeName ? _result = true : _result = false;
			_fbxNode[i].meshNodeName == _biNode[i].meshNodeName ? _result = true : _result = false;

			_fbxNode[i].hasMesh == _biNode[i].hasMesh ? _result = true : _result = false;
			_fbxNode[i].isNegative == _biNode[i].isNegative ? _result = true : _result = false;

			_fbxNode[i].nodeTM == _biNode[i].nodeTM ? _result = true : _result = false;
		}

		for (int i = 0; i < m_pFBXPrefab->m_BoneList.size(); ++i)
		{
			const auto _fbxBone = m_pFBXPrefab->m_BoneList;
			const auto _biBone = m_bineryFrefab->m_BoneList;

			_fbxBone[i].boneName == _biBone[i].boneName ? _result = true : _result = false;
			_fbxBone[i].parentboneName == _biBone[i].parentboneName ? _result = true : _result = false;

			_fbxBone[i].parentBoneIndex == _biBone[i].parentBoneIndex ? _result = true : _result = false;

			_fbxBone[i].m_WorldTM == _biBone[i].m_WorldTM ? _result = true : _result = false;
			_fbxBone[i].m_bone_From3DSceneSpaceToBoneSpaceTM == _biBone[i].m_bone_From3DSceneSpaceToBoneSpaceTM ? _result = true : _result = false;
		}

		for (int i = 0; i < m_pFBXPrefab->m_AnimList.size(); ++i)
		{
			const auto _fbxAnim = m_pFBXPrefab->m_AnimList;
			const auto _biAnim = m_bineryFrefab->m_AnimList;

			_fbxAnim[i].name == _biAnim[i].name ? _result = true : _result = false;
			_fbxAnim[i].typeName == _biAnim[i].typeName ? _result = true : _result = false;

			_fbxAnim[i].frameRate == _biAnim[i].frameRate ? _result = true : _result = false;
			_fbxAnim[i].tickPerFrame == _biAnim[i].tickPerFrame ? _result = true : _result = false;
			_fbxAnim[i].totalKeyFrame == _biAnim[i].totalKeyFrame ? _result = true : _result = false;
			_fbxAnim[i].startKeyFrame == _biAnim[i].startKeyFrame ? _result = true : _result = false;
			_fbxAnim[i].endKeyFrame == _biAnim[i].endKeyFrame ? _result = true : _result = false;

			for (int j = 0; j < _fbxAnim[i].keyFrameInfo.size(); ++j)
			{
				_fbxAnim[i].keyFrameInfo[j].nodeName == _biAnim[i].keyFrameInfo[j].nodeName ? _result = true : _result = false;
				if (_result == false) { int a = 0; }
				_fbxAnim[i].keyFrameInfo[j].boneName == _biAnim[i].keyFrameInfo[j].boneName ? _result = true : _result = false;
				if (_result == false) { int a = 0; }

				for (int c = 0; c < _fbxAnim[i].keyFrameInfo[j].animInfo.size(); ++c)
				{
					const auto _fbxAnimInfo = _fbxAnim[i].keyFrameInfo[j].animInfo;
					const auto _biAnimInfo = _biAnim[i].keyFrameInfo[j].animInfo;

					_fbxAnimInfo[c].time == _biAnimInfo[c].time ? _result = true : _result = false;
					if (_result == false) { return _result; }

					_fbxAnimInfo[c].pos == _biAnimInfo[c].pos ? _result = true : _result = false;
					if (_result == false) { return _result; }

					_fbxAnimInfo[c].rot == _biAnimInfo[c].rot ? _result = true : _result = false;
					if (_result == false) { return _result; }

					_fbxAnimInfo[c].scl == _biAnimInfo[c].scl ? _result = true : _result = false;
					if (_result == false) { return _result; }
				}
			}
		}
